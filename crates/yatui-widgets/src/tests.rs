...
use yatui_core::{CursorVisibility, Point, Size};
use yatui_layout::{Dimension, LayoutStyle};
use yatui_render::{FramePatch, PatchCellContent, Renderer};
use yatui_text::{TextBuffer, TextEdit, TextMovement, WidthPolicy};
use yatui_ui::{
...
fn intrinsic_text_starts_in_its_content_box() -> Result<(), Box<dyn Error>> {
    let view = Element::<()>::text("x").layout(LayoutStyle {
        padding: yatui_core::Insets::all(1),
        ..LayoutStyle::default()
    });
    let tree = UiTree::new();
